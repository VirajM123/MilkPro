...
class UiSession extends ChangeNotifier {
  ...
  void signInFromBackend(AppUser user) {
    _currentUser = user;
    notifyListeners();
  }

  // ============================================================
  // OLD LOCAL PREVIEW METHODS
  // KEEP TEMPORARILY UNTIL SALESMAN DETAIL IS CONNECTED
  // ============================================================
  ...
  void signInForRole(
    ...
    if (role == UserRole.salesman) {
      ...
      final salesmen =
          SalesmanUiStore.instance.salesmen;

      final match = salesmen.where(
        (item) =>
  ...
  void refreshSalesmanPermissions(
    String salesmanId,
  ) {
    if (_currentUser?.salesmanId !=
        salesmanId) {
      return;
    }
